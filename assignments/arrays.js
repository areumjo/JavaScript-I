...
// @@ make a function, everytime the dealer just types car id, result comes out
const findCar = function(num) {
    if (num == inventory[num-1].id) {
      let searchResult = inventory[num-1];
      console.log(`Car`, num, `is a`, searchResult.car_year, searchResult.car_make, searchResult.car_model);
    }
  }
findCar(33);
...
for (var i=0; i<inventory.length; i++) {
  newCarList.push(inventory[i].car_model);
  newCarList.sort();
}
console.log(newCarList);

// @@streth goal - use array methods .map()
console.log(inventory.map( a => a.car_model ).sort());
...
// @@stretch goal - use .map()
console.log(inventory.map(a => a.car_year).sort());
...
// @@stretch goal - use .filter()
console.log(inventory.filter( a => a.car_year < 2000).length);
...
// @@stretch goal - use .filter()
console.log(inventory.filter( a => a.car_make === "BMW" || a.car_make === "Audi"));
